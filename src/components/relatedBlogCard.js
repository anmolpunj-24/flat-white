// nextjs import
import Image from "next/image";

export default function RelatedBlogCard({data}) {
  return (
    <div className="flex gap-[10px] p-[14px] rounded-[20px] border border-[#e3e7ee] bg-[#f8fafc] hover:bg-[#eef2f6] transition-all duration-300 cursor-pointer">
      <div className="relative overflow-hidden rounded-[14px] shrink-0 w-[90px] h-[70px]">
        <Image
          src={data?.image}
          alt="related blog"
          fill
          className="object-cover"
        />
      </div>

      <div className="flex flex-col justify-between">
        <h3 className="text-[13px] tracking-[-0.03em] font-[500] text-[#111] leading-[1.1em]">
          {data?.title}
        </h3>

        <span className="text-[12px] text-[#7b8494]">{data?.date}</span>
      </div>
    </div>
  );
}
